...
public class TCPFrameChannel implements AutoCloseable {
    ...
    private final ByteBuffer lenBuf;
    ...
    public TCPFrameChannel(ChannelIO channelIO) {
        ...
        this.lenBuf = ByteBuffer.allocate(LEN_BYTES).order(ByteOrder.BIG_ENDIAN);
        this.staging = ByteBuffer.allocateDirect(4 * 1024 * 1024).order(ByteOrder.BIG_ENDIAN);
    }

    @Override
    public void close() {
        // Clear internal state; does not close underlying IO.
        staging.clear();
        lenBuf.clear();
        this.frameBuf = null;
    }
